...
# Automated benchmark (CFS vs db_aware)
...
# Automated A/B benchmark (builds nginx + wrk2 + scheduler if needed)
...
└── db_sim_bench.py       # Automated benchmark (CFS vs db_aware)
...
├── redis_bench_compare.sh # Automated 3-run A/B comparison
└── redis-src/            # Redis source (git submodule)

workloads/nginx/
├── nginx_aware.bpf.c      # BPF scheduler: asymmetric + task local storage
